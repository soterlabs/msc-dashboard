...
/**
 * Addition to upstream: Base UI publishes the active tab's box as
 * --active-tab-left/width/top/height, so the fill can be one element that
 * travels rather than one that blinks. shadcn does not ship this part.
 */
function TabsIndicator({ className, ...props }: TabsPrimitive.Indicator.Props) {
  return (
    <TabsPrimitive.Indicator
      data-slot="tabs-indicator"
      // Without this the indicator has no measurements until React hydrates,
      // so the selected tab renders unfilled for a frame.
      renderBeforeHydration
      className={cn(
        "absolute top-(--active-tab-top) left-(--active-tab-left) h-(--active-tab-height) w-(--active-tab-width) rounded-full transition-[left,width,top,height] duration-200 ease-out motion-reduce:transition-none",
        className
      )}
      {...props}
    />
