/**
 * Regenerates src/lib/{data,ssr-data,prime-data}.ts from the sources of truth:
 *
 ...
 *   - data/prime/payments.csv (local, see its README)                                   → prime-data.ts
 *
...
import { PRIME_PAYMENTS_CSV, readPrimePayments } from "./lib/prime-payments.mjs";
...
function generatePrime() {
  ...
  // Add a wallet to PRIME_TRANSFER_WALLETS when a new foundation/msig
 ...
 *
 * GENERATED by scripts/generate-data.mjs from data/prime/payments.csv.
 * Do not edit by hand — run \`pnpm generate-data\` instead.
 *
...
function main() {
  ...
  if (process.argv.includes("--prime-only")) {
    fs.writeFileSync(primeOut, generatePrime());
    console.log(
      `[generate-data] wrote ${path.relative(ROOT, primeOut)} from ${path.relative(ROOT, PRIME_PAYMENTS_CSV)} (--prime-only)`,
    );
    return;
  }
