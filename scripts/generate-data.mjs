...
function generatePrime(root) {
  ...
  if (!table) throw new Error("prime-payments md has no markdown table");
  const header = table.header.map(str);
  const idx = PRIME_COLUMNS.map(([col]) => header.indexOf(col));
  const missing = PRIME_COLUMNS.filter((_, i) => idx[i] === -1).map(([col]) => col);
  if (missing.length) {
    throw new Error(`prime-payments md header changed — columns not found: ${missing.join(", ")}`);
  }
...
export const primePayments: PrimePayment[] = ${json(payments)};
`;
}

// ---------------------------------------------------------------- main

const primeOut = path.join(ROOT, "src/lib/prime-data.ts");

// Prime payments come from a local markdown file, so they can be rebuilt
// without the private DR/SSR repos (which need SSH).
if (process.argv.includes("--prime-only")) {
  fs.writeFileSync(primeOut, generatePrime(ROOT));
  console.log(`[generate-data] wrote ${path.relative(ROOT, primeOut)} (--prime-only)`);
  process.exit(0);
}
...
const primeTs = generatePrime(ROOT);
...
fs.writeFileSync(primeOut, primeTs);
console.log(`[generate-data] wrote ${path.relative(ROOT, primeOut)}`);
