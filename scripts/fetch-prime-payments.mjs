...
// prime-data.ts is regenerated from the markdown by generate-data.mjs, the
// single owner of the .md → .ts step (incl. derived fields like walletType).
// Delegating here keeps this script from re-implementing — and drifting from —
// that parsing/derivation logic.
function writeTs() {
...
async function main() {
  ...
  console.log(
    `[fetch-prime-payments] ${merged.length} rows → ${path.relative(ROOT, MD)} (+ regenerated prime-data.ts)` +
